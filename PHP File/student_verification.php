...
if (isset($_POST['approve'])) {
    ...
    if ($conn->query($update_sql)) {
        // Fetch student email
        $student_sql = "SELECT email FROM students WHERE id = $student_id";
        $student_result = $conn->query($student_sql);
        if ($student = $student_result->fetch_assoc()) {
            $to = $student['email'];
            $subject = "Your Bus Pass Application is Approved!";
            $message = "Dear Student,\n\nCongratulations! Your bus pass application has been approved.\n\nThank you.";
            $headers = "From: your-himanshugirhepunje4817@gmail.com"; // <-- apna email daalna
